...
# For mortality temperature deactivation does not make sense, so we set it to zero 
   ...
   # For metabolism temperature deactivation does not make sense, so we set it to zero 
   if (!("ed_met" %in% colnames(object))) {
     message("Note: \tNo ed_met column provided, setting deactivation energy for mortality to 0, giving exponential increase only")
     object$ed_met <- 0
   }
   missing <- is.na(object$ed_met)
   if (any(missing)) {
     object$ed_met[missing] <- 0
   }
